package de.tud.evaluation;

import com.google.gson.Gson;

public class EvaluationConstants {
	
	public static final boolean USE_GUI = false;
	public static final Gson gson = new Gson();

	/*public static int NUMBER_EXPLORE_AGENTS = 1;
	public static int NUMBER_WIPE_AGENTS = 0;
	public static int NUMBER_HOOVE_AGENTS = 0;
	public static int NEW_FIELD_COUNT = 0;
	public static String map = "R"; //F...Fakultät R...Rechteck L...Labyrinth	
	public static int run = 0;	
	public static int configuration = 1;
	
	public static int iteration = 0;
	public static List<ExchangeMeasurement> exchange = new LinkedList<ExchangeMeasurement>();*/
}
