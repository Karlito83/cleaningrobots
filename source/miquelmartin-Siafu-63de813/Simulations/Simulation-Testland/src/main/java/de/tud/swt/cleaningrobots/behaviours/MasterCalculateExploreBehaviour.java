...
public class MasterCalculateExploreBehaviour extends Behaviour{
	...
	public MasterCalculateExploreBehaviour(RobotCore robot, MasterRole mr) {
		super(robot);
		...
		for (RobotCore oneRobot : allRobots) {
			...
			for (RobotDestinationCalculation rdc : information.values()) {
				if (oneRobot.getName().equals(rdc.getName())) 
				{
					...
					{						
						Position nextUnknownPosition = this.getRobot().getWorld().getNextUnknownRelativeFieldPosition(rdc.actualPosition, rdc.oldDest); 
